...
from models import Pago, EstructuraPago, Alumno, AbonoPago
from datetime import datetime
from decimal import Decimal  # 🔥 IMPORTANTE PARA LA PRECISIÓN FINANCIERA
...
def historial_pagos(matricula):
    ...
    try:
        ...
        for p, e in historial:
            ...
            # 🔥 OBTENEMOS EL HISTORIAL DE ABONOS DE ESTE PAGO
            abonos_db = AbonoPago.query.filter_by(id_pago=p.id_pago).order_by(AbonoPago.fecha_abono.asc()).all()
            lista_abonos = [{
                "fecha": a.fecha_abono.strftime("%Y-%m-%d"),
                "folio": a.folio,
            ...
            resultado.append({
                ...
                "estado": estado_str.split('.')[-1],
                "fecha_pago": fecha_formateada, 
                "fecha_raw": fecha_raw, 
                "folio": p.folio,
                "abonos": lista_abonos, # ✅ Pasamos la lista de abonos a Angular
                "mes": getattr(e, 'mes', None),
                "anio": getattr(e, 'anio', None),
                "oportunidad": getattr(e, 'oportunidad', None)  # ← esto
...
def revertir_pago(id_pago):
    ...
    try:
        ...
        # 🔥 ELIMINAR LOS ABONOS EN CASCADA
        abonos = AbonoPago.query.filter_by(id_pago=id_pago).all()
        for abono in abonos:
        ...
        return jsonify({"message": "Pago cancelado exitosamente y abonos revertidos"}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
...
def registrar_cobro_oficial():
    ...
    if not fecha_str or not folio_ingresado:
        return jsonify({"error": "La fecha y el folio son obligatorios"}), 400

    # 🔥 PARSEO EXACTO PARA EVITAR ERRORES DE PYTHON A MYSQL
    ...
    try:
        ...
        # Validación inteligente en la tabla AbonoPago
        # DESPUÉS ✅
        folio_existente = AbonoPago.query.join(Pago).filter(
            AbonoPago.folio == folio_ingresado,
            Pago.id_alumno != pago.id_alumno  # Solo bloquea si es OTRO alumno
        ).first()
        if folio_existente:
            return jsonify({"error": f"El folio '{folio_ingresado}' ya está registrado para otro alumno."}), 400
        
        estructura = EstructuraPago.query.get(pago.id_estructura)
        costo_total = Decimal(str(estructura.monto))
        
        # Sumamos con precisión utilizando Decimal
        total_acumulado_db = db.session.query(db.func.sum(AbonoPago.monto_abono)).filter_by(id_pago=id_pago).scalar() or Decimal('0.00')
        nuevo_acumulado = total_acumulado_db + monto_recibido
        
        if nuevo_acumulado > costo_total:
             return jsonify({"error": f"El monto excede el total. Restante válido: ${costo_total - total_acumulado_db:.2f}"}), 400

        # ✅ 1. Registramos el abono detallado
        nuevo_abono = AbonoPago(
            id_pago=pago.id_pago,
            monto_abono=monto_recibido,
            fecha_abono=fecha_ingresada,
            folio=folio_ingresado
        )
        db.session.add(nuevo_abono)

        # ✅ 2. Actualizamos el registro padre
        estado_nuevo = 'PAGADO' if nuevo_acumulado >= costo_total else 'PARCIAL'

        if hasattr(pago, 'monto_abonado'): pago.monto_abonado = float(nuevo_acumulado)
...
def crear_pago_manual():
    ...
    try:
        ...
        # 🔥 SOLUCIÓN: Desvincular cargos individuales de la generación global
        # Si es un EEE o EXT, asignamos None a la generación para que no se herede a alumnos nuevos.
        gen_id = None if tipo_recibido in ['EEE', 'EXT'] else alumno.id_generacion

        ahora = datetime.now()
        nueva_estructura = EstructuraPago(
            id_generacion=gen_id, # <--- Se asigna la variable dinámica
            tipo=tipo_recibido,
            ...
            oportunidad=data.get('oportunidad', '1ra Oportunidad')  # ← esto
...
def editar_pago(id_pago):
    ...
    try:
        ...
        if estructura:
            ...
            # 🔥 CORRECCIÓN: Actualizar oportunidad, mes y año
            if "oportunidad" in data:
                estructura.oportunidad = data["oportunidad"]
            if "mes" in data:
                estructura.mes = data["mes"]
            if "anio" in data:
                estructura.anio = data["anio"]
        ...
        if estado_pago in ['PAGADO', 'PARCIAL']:
            ...
            # 🔥 SINCRONIZACIÓN AL EDITAR: Actualizamos el último abono registrado
            ultimo_abono = AbonoPago.query.filter_by(id_pago=id_pago).order_by(AbonoPago.id_abono.desc()).first()
            if ultimo_abono:
                if "fecha" in data: ultimo_abono.fecha_abono = datetime.strptime(data["fecha"], "%Y-%m-%d").date()
                if "folio" in data: ultimo_abono.folio = data["folio"]
...
# 🎁 API: EXTENTAR
# ==========================================
# 🔥 CORRECCIÓN AQUÍ: Se agregó /caja a la ruta
@caja_bp.route("/caja/exentar/<int:id_pago>", methods=["PUT", "OPTIONS"])
def exentar_pago(id_pago):
    if request.method == "OPTIONS": return jsonify({}), 200
    ...
    operador = get_jwt()
    if operador.get('rol') not in ['ADMIN', 'SISTEMAS']: 
        return jsonify({"error": "No tienes permisos para aplicar becas"}), 403
    ...
    try:
        ...
        registrar_accion(
            id_usuario=obtener_id_admin(),
            accion="APLICAR_BECA",
            descripcion=f"Exentó el pago ID {id_pago} (Beca aplicada)"
        )
        
        return jsonify({"message": "El cobro ha sido cubierto por beca/exención."}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
