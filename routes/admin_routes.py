...
def gestionar_usuarios():
    ...
    operador = get_jwt()
    rol_operador = operador.get('rol')

    if request.method == "GET":
        if rol_operador not in ['SISTEMAS', 'ADMIN', 'LECTURA']:
    ...
    if request.method == "POST":
        if rol_operador not in ['SISTEMAS', 'ADMIN']:
...
# ==========================================
# 📝 EDITAR Y 🗑️ ELIMINAR PERSONAL
# ==========================================
@admin_bp.route("/usuarios/<int:id_usuario>", methods=["PUT", "DELETE", "OPTIONS"])
def manejar_usuario_individual(id_usuario):
    if request.method == "OPTIONS": return jsonify({}), 200
    ...
    # ----------------------------------------
    # PETICIÓN PUT: EDITAR USUARIO
    # ----------------------------------------
    if request.method == "PUT":
        if rol_operador not in ['SISTEMAS', 'ADMIN']:
            return jsonify({"error": "No tienes permisos para editar usuarios"}), 403
        ...
        try:
            ...
            # Verificar que el nuevo correo no le pertenezca a otro usuario
            correo_existente = Usuario.query.filter(Usuario.correo == data.get("correo"), Usuario.id_usuario != id_usuario).first()
            if correo_existente:
                return jsonify({"error": "El correo ya está en uso por otra cuenta"}), 400
            ...
            registrar_accion(
                ...
                descripcion=f"Editó la información del usuario: {usuario.correo}"
            )
    ...
    # ----------------------------------------
        ...
        if rol_operador != 'SISTEMAS':
            return jsonify({"error": "Solo el nivel SISTEMAS puede eliminar usuarios"}), 403
...
# ==========================================
# 🔌 CAMBIAR ESTADO (DAR DE BAJA / ACTIVAR)
# ==========================================
@admin_bp.route("/usuarios/<int:id_usuario>/estado", methods=["PUT", "OPTIONS"])
def cambiar_estado_usuario(id_usuario):
    ...
    try:
        # 🔥 EL ARREGLO ESTÁ AQUÍ: Forzamos el UPDATE directo a la Base de Datos 🔥
        Usuario.query.filter_by(id_usuario=id_usuario).update({"estado": nuevo_estado})
        ...
        registrar_accion(
            ...
            descripcion=f"Cambió el estado del ID {id_usuario} a {nuevo_estado}"
        )
...
def obtener_datos_dashboard():
    ...
    try:
        ...
        dia_actual = hoy.day  # 🔥 NUEVO: Necesario para calcular los 5 días de gracia
        ...
        for i in range(5, -1, -1):
            ...
            for p, e in pagos_completos:
                estado_pago = str(p.estado).split('.')[-1].upper()
                monto_real = float(p.monto_abonado) if p.monto_abonado else float(e.monto or 0)
                
                if (estado_pago in ['PAGADO', 'PARCIAL'] or monto_real > 0) and p.fecha_pago:
                    if p.fecha_pago.month == mes_calc and p.fecha_pago.year == anio_calc:
                        suma_mes += monto_real
            
            ingresos_data.append(suma_mes)

        # 🔥 CÁLCULO REAL Y SINCRONIZADO DE ADEUDOS
        resumen_alumnos = []

        for a in alumnos:
            if a.estatus == 'ACTIVO':
                tiene_deuda = False
                sem_alumno = a.semestre_actual or 1
                
                # Solo traemos los recibos que NO están totalmente pagados
                ...
                for p, e in pagos_pendientes:
                    # 1. Cargos extras/EEE siempre son deuda si no están pagados
                    if e.tipo in ['EEE', 'EXT']:
                        tiene_deuda = True
                        break
                    
                    # 2. Deudas arrastradas de semestres anteriores
                    if e.semestre < sem_alumno:
                        tiene_deuda = True
                        break
                    
                    # 3. Inscripción del semestre EN CURSO
                    if e.tipo == 'INSCRIPCION' and e.semestre == sem_alumno:
                        tiene_deuda = True
                        break
                    
                    # 🔥 IGNORAMOS EL FUTURO: Si el recibo es de un semestre que el alumno aún no cursa
                    if e.semestre > sem_alumno:
                        continue
                    
                    # 4. Mensualidades: evaluamos por mes, año y los 5 días de gracia
                    if e.mes and e.anio:
                        diferencia_meses = (e.anio - anio_actual) * 12 + (e.mes - mes_actual)
                        
                        if diferencia_meses < 0: # Meses anteriores vencidos
                            tiene_deuda = True
                            break
                        elif diferencia_meses == 0 and dia_actual > 5: # Mes en curso (después del día 5)
                            tiene_deuda = True
                            break
...
def gestionar_generaciones_grupos():
    ...
    if request.method == "POST":
        if rol_operador not in ['SISTEMAS', 'ADMIN']:
            return jsonify({"error": "No tienes permisos para crear generaciones o grupos"}), 403
